...
| 🌑 | **显示器省电** | 让合盖后的内置面板保持黑暗，并可选择让外接屏幕也休眠。 |
...
Sleepless 切换并读回 `pmset disablesleep`（内核的 `SleepDisabled` 标志）；应用运行期间会在达到电量下限、进入 Low Power Mode、定时器结束或正常退出时还原它。合盖时，它会先保存内置面板的硬件亮度并降为零；若没有外接屏幕，再请求真正的显示器休眠。外接屏幕默认保持开启；打开**同时让外接屏幕休眠**后则会让所有屏幕休眠。应用不会监控键盘或指针事件。亮度处理使用两个未公开的 Apple 函数；若函数不可用，也不会改动外接屏幕。睡眠设置对外接电源和电池供电都全局生效；“使用电池且没有外接显示器”描述的是它支持的用法，而不是应用检查的条件。重启也会重置。GUI 应用没法输入密码，所以安装程序会加一条范围严格限定的 sudoers 规则，**只允许两条命令**：
